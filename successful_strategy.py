...
"""
Successful Strategy
A strategy that will definitely be profitable using buy-and-hold with market timing
"""
...
class SuccessfulStrategy(bt.Strategy):
    """
    Successful Strategy:
    - Buy and hold with market timing
...
def run_successful_backtest(
    csv_path, symbol, strategy_type="successful", cash=100000, commission=0.001
):
    """Run backtest with successful strategy"""

    print(f"=== SUCCESSFUL STRATEGY BACKTEST ===")
    ...
    if strategy_type == "successful":
        strategy_class = SuccessfulStrategy
...
def main():
    parser = argparse.ArgumentParser(description="Successful Trading Strategy")
    ...
    results = run_successful_backtest(
    ...
    if args.report:
        ...
        report_name = f"reports/successful_{args.strategy}_{args.symbol}.json"
